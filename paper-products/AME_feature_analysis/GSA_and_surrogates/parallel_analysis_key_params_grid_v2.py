...
if __name__ == "__main__":
    ...
    # sigma_list = np.random.uniform(0.01, 5, N_samples)
    ...
    log_pwidth_list = np.random.uniform(np.log(0.1), np.log(20), N_samples)

    param_array_training = np.column_stack((log_pwidth_list, gamma_list)) 
    ...
    log_beta0, delta = np.log(0.05), 0.1  # Fixed beta distribution parameters
    log_a0 = 0.5 * (np.log(MC_env.a_min) + np.log(MC_env.a_max))
    log_pc = 0.5 * (np.log(MC_env.xC_min) + np.log(MC_env.xC_max))
    sigma = 0.5
    params_tab = MC_env.a_beta_env_tab
    moments_training = []
    SED_array_training = []
    ...
    for i in tqdm.tqdm(range(N_samples)):
        aux_func = a_beta_env_dist(gamma_list[i], log_a0, sigma, log_beta0, delta, log_pc, log_pwidth_list[i])
        prob_weights = aux_func(params_tab)
    ...
    np.savetxt("data/MC_emu/MC_dist_Mom.txt", moments_training)
    np.savetxt("data/MC_emu/MC_SED_feature.txt", features_training)
    np.savetxt("data/MC_emu/MC_dist_params.txt", param_array_training)
    np.savetxt("data/MC_emu/MC_SED_logmoments.txt", SED_log_moments)
